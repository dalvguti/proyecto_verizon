data = 
    '{' +
        '"image-folder": "./assets/img/",' +
        '"video-folder": "./assets/videos/",' +
        '"audio-folder": "./assets/audios/",' +
        '"subtitle-folder": "./assets/subtitles/",' +
        '"image-home": "home.png",' +
        '"title": "Título del documento",' +
        '"back-button": "BACK",' +
        '"bottom-menu": {' +
            '"guide-button": "Tour Guide",' +
            '"pause-guide-button": "Pause Guide"' +
        '},' +
        '"buildings":[' +
            '{' +
                '"element-name":"building1",' +
                '"page-number":2,' +
                '"transition": "to_convention_center.mp4",' +
                '"transition-time": 1700,' +
                '"name":"Convention Center",' +
                '"name-top":"10%",' +
                '"name-left":"35%",' +
                '"class-name":"building-label-bottom-right",' +
                '"top":"17%",' +
                '"left":"5%",' +
                '"transform":30,' +
                '"height":"(1.2*(imageWidth/10))",' +
                '"width":"(1.6*(imageWidth/10))"' +                
            '},' +
            '{' +
                '"element-name":"building2",' +
                '"page-number":1,' +
                '"transition": "none.gif",' +
                '"transition-time": 1700,' +
                '"name":"Corporate Office",' +
                '"name-top":"90%",' +
                '"name-left":"40%",' +
                '"class-name":"building-label-top-right",' +
                '"top":"8%",' +
                '"left":"33%",' +
                '"transform":0,' +
                '"height":"(1.5*(imageWidth/10))",' +
                '"width":"(1.5*(imageWidth/10))"' +               
            '},' +
            '{' +
                '"element-name":"building3",' +
                '"page-number":1,' +
                '"transition": "none.gif",' +
                '"transition-time": 1700,' +
                '"name":"Healthcare",' +
                '"name-top":"0%",' +
                '"name-left":"5%",' +
                '"class-name":"building-label-bottom-right",' +
                '"top":"10%",' +
                '"left":"55%",' +
                '"transform":0,' +
                '"height":"(0.7*(imageWidth/10))",' +
                '"width":"(1.1*(imageWidth/10))"' +               
            '},' +
            '{' +
                '"element-name":"building4",' +
                '"page-number":1,' +
                '"transition": "none.gif",' +
                '"transition-time": 1700,' +
                '"name":"Retail",' +
                '"name-top":"90%",' +
                '"name-left":"35%",' +
                '"class-name":"building-label-top-right",' +
                '"top":"30%",' +
                '"left":"55%",' +
                '"transform":25,' +
                '"height":"(0.7*(imageWidth/10))",' +
                '"width":"(1.4*(imageWidth/10))"' +              
            '},' +
            '{' +
                '"element-name":"building5",' +
                '"page-number":1,' +
                '"transition": "none.gif",' +
                '"transition-time": 1700,' +
                '"name":"Ports",' +
                '"name-top":"0%",' +
                '"name-left":"50%",' +
                '"class-name":"building-label-bottom-right",' +
                '"top":"65%",' +
                '"left":"28%",' +
                '"transform":-30,' +
                '"height":"(1.2*(imageWidth/10))",' +
                '"width":"(2.5*(imageWidth/10))"' +               
            '},' +
            '{' +
                '"element-name":"building6",' +
                '"page-number":1,' +
                '"transition": "none.gif",' +
                '"transition-time": 1700,' +
                '"name":"Manufacturing",' +
                '"name-top":"0%",' +
                '"name-left":"50%",' +
                '"class-name":"building-label-bottom-right",' +
                '"top":"50%",' +
                '"left":"8%",' +
                '"transform":-30,' +
                '"height":"(1.2*(imageWidth/10))",' +
                '"width":"(2*(imageWidth/10))"' +                
            '}' +
        '],' + 
        '"pages":[' +
            '{' +
                '"page-number":1,' +
                '"page-number-next":2,' +
                '"page-number-previous":0,' +
                '"page-building":"home",' +
                '"page-img":"home.png",' +
                '"page-audio":"InitialHome.ogg",' +
                '"page-subtitles":{' +
                    '"type":"audio",' +
                    '"file":"InitialHome.vtt"' +
                '},' +
                '"page-name":"HomePage",' +
                '"page-title":"Welcome to the connected city of the future.",' +
                '"page-description":[' +
                    '{"0":"Discover how Verizon\'s innovative solutions are transforming industries and delivering meaningful outcomes for customers."},' +
                    '{"1":"Click any building to learn about our business solutions."},' +
                    '{"2":"Or Click on a business outcome below to discover which industries are harnessing Verizon technologies."}' +
                '],' +
                '"menu-items": [' +
                    '{' +
                        '"item-name": "Business Outcome 1",' +
                        '"item-image": "Business_Outcome_1_00000.png",' +
                        '"transition-time": 5000' +                
                    '},' + 
                    '{' +
                        '"item-name": "Business Outcome 2",' +
                        '"item-image": "Business_Outcome_2_00000.png",' +
                        '"transition-time": 5000' +                
                    '},' + 
                    '{' +
                        '"item-name": "Business Outcome 3",' +
                        '"item-image": "Business_Outcome_3_00000.png",' +
                        '"transition-time": 5000' +                 
                    '},' + 
                    '{' +
                        '"item-name": "Business Outcome 4",' +
                        '"item-image": "Business_Outcome_4_00000.png",' +
                        '"transition-time": 5000' +                 
                    '},' + 
                    '{' +
                        '"item-name": "Business Outcome 5",' +
                        '"item-image": "Business_Outcome_5_00000.png",' +
                        '"transition-time": 5000' +                 
                    '},' + 
                    '{' +
                        '"item-name": "Business Outcome 6",' +
                        '"item-image": "Business_Outcome_6_00000.png",' +
                        '"transition-time": 5000' +                 
                    '}' + 
                ']' +
            '},' +
            '{' +
                '"page-number":2,' +
                '"page-number-next":3,' +
                '"page-number-previous":1,' +
                '"page-building":"building1",' +
                '"page-img":"convention_center.png",' +
                '"page-audio":"ConventionCenter_Building.ogg",' +
                '"page-subtitles":{' +
                    '"type":"audio",' +
                    '"file":"ConventionCenter_Building.vtt"' +
                '},' +
                '"page-name":"Convention Center",' +
                '"page-title":"Convention Center home screen",' +
                '"page-transition-out":"from_convention_center.mp4",' +
                '"page-transition-time":1700,' +
                '"page-description":[' +
                    '{"0":"Convention centers of the future enhance connectivity and provide quicker, simpler access to services and amenities."},' +
                    '{"1":"Select a demo to learn more."}' +
                '],' +
                '"menu-items": [' +
                    '{' +
                        '"item-name": "Wireless Mutli-Carrier Connectivity",' +
                        '"item-image":"TO_MultiCarrier.mp4",' +
                        '"transition-time": 1000,' +             
                        '"page-number-next":3' +                
                    '},' + 
                    '{' +
                        '"item-name": "Cashierless Checkout",' +
                        '"item-image":"TO_Cashierless.mp4",' +
                        '"transition-time": 2500,' +                          
                        '"page-number-next":13' +           
                    '},' +  
                    '{' +
                        '"item-name": "Accelerated Access",' +
                        '"page-number-next":0' +           
                    '}' + 
                '],' +
                '"page-title-problem":"None",' +
                '"page-description-problem":[],' +
                '"menu-items-problem": []' +
            '},' +
            '{' +
                '"page-number":3,' +
                '"page-number-next":4,' +
                '"page-number-previous":2,' +
                '"page-building":"building1",' +
                '"page-img":"convention_center.png",' +
                '"page-name":"Convention Center",' +
                '"page-transition-out":"FROM_MultiCarrier.mp4",' +
                '"page-transition-time":1000,' +
                '"page-title":"WirelessMulti-Carrier Connectivity",' +
                '"page-description":[],' +
                '"menu-items": [' +
                    '{' +
                        '"item-name": "Neutral Host Network",' +
                        '"page-number-next":9' +                
                    '},' + 
                    '{' +
                        '"item-name": "Private Wireless Solution",' +
                        '"page-number-next":11' +           
                    '}' +  
                '],' +
                '"page-title-problem":"The Problem with Legacy Convention Centers",' +
                '"page-description-problem":[],' +
                '"menu-items-problem": [' +
                    '{' +
                        '"item-name": "Spotty Coverage",' +
                        '"page-number-next":4' +                
                    '},' + 
                    '{' +
                        '"item-name": "Dropped Calls",' +
                        '"item-image":"TO_DroppedCalls.mp4",' + 
                        '"transition-time": 2000,' +  
                        '"page-number-next":5' +           
                    '},' + 
                    '{' +
                        '"item-name": "Limited Data Access",' +
                        '"item-image":"TO_LimitedData.mp4",' +  
                        '"transition-time": 2000,' +  
                        '"page-number-next":6' +           
                    '},' +  
                    '{' +
                        '"item-name": "Device Latency",' +
                        '"item-image":"TO_DeviceLatency.mp4",' + 
                        '"transition-time": 2000,' +  
                        '"page-number-next":7' +           
                    '},' +  
                    '{' +
                        '"item-name": "Reduced Security",' +
                        '"item-image":"TO_ReducedSecurity.mp4",' + 
                        '"transition-time": 2000,' +  
                        '"page-number-next":8' +           
                    '}' +   
                ']' +
            '},' +
            '{' +
                '"page-number":4,' +
                '"page-number-next":0,' +
                '"page-number-previous":3,' +
                '"page-building":"building1",' +
                '"page-img":"SpottyConnectivity_Infinite.mp4",' +
                '"page-subtitles":{' +
                    '"type":"video",' +
                    '"file":"SpottyConnectivity_Infinite.vtt"' +
                '},' +
                '"page-name":"Convention Center",' +
                '"page-title":"None",' +
                '"page-description":[],' +
                '"menu-items": [],' +
                '"page-title-problem":"None",' +
                '"page-description-problem":[],' +
                '"menu-items-problem": []' +
            '},' +
            '{' +
                '"page-number":5,' +
                '"page-number-next":0,' +
                '"page-number-previous":3,' +
                '"page-building":"building1",' +
                '"page-img":"DroppedCalls.mp4",' +
                '"page-subtitles":{' +
                    '"type":"video",' +
                    '"file":"DroppedCalls.vtt"' +
                '},' +
                '"page-name":"Convention Center",' +
                '"page-title":"None",' +
                '"page-transition-out":"FROM_DroppedCalls.mp4",' +
                '"page-transition-time":1700,' +
                '"page-description":[' +
                '],' +
                '"menu-items": [],' +
                '"page-title-problem":"None",' +
                '"page-description-problem":[],' +
                '"menu-items-problem": []' +
            '},' +
            '{' +
                '"page-number":6,' +
                '"page-number-next":0,' +
                '"page-number-previous":3,' +
                '"page-building":"building1",' +
                '"page-img":"LimitedData.mp4",' +
                '"page-subtitles":{' +
                    '"type":"video",' +
                    '"file":"LimitedData.vtt"' +
                '},' +
                '"page-name":"Convention Center",' +
                '"page-title":"None",' +
                '"page-transition-out":"FROM_LimitedData.mp4",' +
                '"page-transition-time":1700,' +
                '"page-description":[' +
                '],' +
                '"menu-items": [],' +
                '"page-title-problem":"None",' +
                '"page-description-problem":[],' +
                '"menu-items-problem": []' +
            '},' +
            '{' +
                '"page-number":7,' +
                '"page-number-next":0,' +
                '"page-number-previous":3,' +
                '"page-building":"building1",' +
                '"page-img":"DeviceLatency.mp4",' +
                '"page-subtitles":{' +
                    '"type":"video",' +
                    '"file":"DeviceLatency.vtt"' +
                '},' +
                '"page-name":"Convention Center",' +
                '"page-title":"None",' +
                '"page-transition-out":"FROM_DeviceLatency.mp4",' +
                '"page-transition-time":1700,' +
                '"page-description":[' +
                '],' +
                '"menu-items": [],' +
                '"page-title-problem":"None",' +
                '"page-description-problem":[],' +
                '"menu-items-problem": []' +
            '},' +
            '{' +
                '"page-number":8,' +
                '"page-number-next":0,' +
                '"page-number-previous":3,' +
                '"page-building":"building1",' +
                '"page-img":"ReducedSecurity.mp4",' +
                '"page-subtitles":{' +
                    '"type":"video",' +
                    '"file":"ReducedSecurity.vtt"' +
                '},' +
                '"page-name":"Convention Center",' +
                '"page-title":"None",' +
                '"page-transition-out":"FROM_ReducedSecurity.mp4",' +
                '"page-transition-time":1700,' +
                '"page-description":[' +
                '],' +
                '"menu-items": [],' +
                '"page-title-problem":"None",' +
                '"page-description-problem":[],' +
                '"menu-items-problem": []' +
            '},' +
            '{' +
                '"page-number":9,' +
                '"page-number-next":10,' +
                '"page-number-previous":3,' +
                '"page-building":"building1",' +
                '"page-img":"VerizonNeutralHost_Infinite.mp4",' +
                '"page-name":"Convention Center",' +
                '"page-title":"Neutral Host Network",' +
                '"page-description":[' +
                    '{"0":"Ensures seamless universal access for atendees and exhibitors."},' +
                    '{"1":"Enhances user experience and supports efficient business operation."},' +
                    '{"2":"Sets a new standard for digital excellence in event spaces."}' +
                '],' +
                '"menu-items": [' +
                    '{' +
                        '"item-name": "Click Here to Zoom in on the Solution",' +
                        '"page-number-next":10' +                
                    '}' +  
                '],' +
                '"page-title-problem":"",' +
                '"page-description-problem":[],' +
                '"menu-items-problem": [' +
                ']' +
            '},' +
            '{' +
                '"page-number":10,' +
                '"page-number-next":0,' +
                '"page-number-previous":9,' +
                '"page-building":"building1",' +
                '"page-img":"VerizonNeutralHost.mp4",' +
                '"page-subtitles":{' +
                    '"type":"video",' +
                    '"file":"VerizonNeutralHost.vtt"' +
                '},' +
                '"page-name":"Convention Center",' +
                '"page-title":"None",' +
                '"page-description":[' +
                '],' +
                '"menu-items": [],' +
                '"page-title-problem":"None",' +
                '"page-description-problem":[],' +
                '"menu-items-problem": []' +
            '},' +
            '{' +
                '"page-number":11,' +
                '"page-number-next":12,' +
                '"page-number-previous":3,' +
                '"page-building":"building1",' +
                '"page-img":"PrivateNetwork_Infinite.mp4",' +
                '"page-name":"Convention Center",' +
                '"page-title":"Private Wireless Solution",' +
                '"page-description":[' +
                    '{"0":"Private connections for employees and venue devices."},' +
                    '{"1":"Terminates on a private wireless core."},' +
                    '{"2":"Sets a new standard for digital excellence in event spaces."},' +
                    '{"3":"Fast separate and secure network environment separate from public carriers."}' +
                '],' +
                '"menu-items": [' +
                        '{' +
                            '"item-name": "Click Here to Zoom Into the PWN",' +
                            '"page-number-next":12' +                
                        '}' +  
                '],' +
                '"page-title-problem":"",' +
                '"page-description-problem":[],' +
                '"menu-items-problem": [' +
                ']' +
            '},' +
            '{' +
                '"page-number":12,' +
                '"page-number-next":0,' +
                '"page-number-previous":11,' +
                '"page-building":"building1",' +
                '"page-img":"PrivateNetwork.mp4",' +
                '"page-subtitles":{' +
                    '"type":"video",' +
                    '"file":"PrivateNetwork.vtt"' +
                '},' +
                '"page-name":"Convention Center",' +
                '"page-title":"None",' +
                '"page-description":[' +
                '],' +
                '"menu-items": [],' +
                '"page-title-problem":"None",' +
                '"page-description-problem":[],' +
                '"menu-items-problem": []' +
            '},' +
            '{' +
                '"page-number":13,' +
                '"page-number-next":14,' +
                '"page-number-previous":2,' +
                '"page-building":"building1",' +
                '"page-img":"Cashierless.mp4",' +
                '"page-name":"Convention Center",' +
                '"menu-left": true,' +
                '"page-title":"Solution",' +
                '"page-transition-out":"FROM_Cashierless.mp4",' +
                '"page-transition-time":1700,' +
                '"page-description":[],' +
                '"menu-items": [' +
                    '{' +
                        '"item-name": "Cashierless Checkout",' +   
                        '"page-number-next":15' +                
                    '}' + 
                '],' +
                '"page-title-problem":"The Problem",' +
                '"page-description-problem":[],' +
                '"menu-items-problem": [' +
                    '{' +
                        '"item-name": "Traditional Checkout",' +
                        '"page-number-next":14' +                
                    '}' + 
                '],' +
                '"page-title-work":"How it works",' +
                '"page-description-work":[],' +
                '"menu-items-work": [' +
                    '{' +
                        '"item-name": "Authentication and Entry",' +
                        '"item-image":"TO_Authentication.mp4",' +
                        '"transition-time": 1000,' +                  
                        '"page-number-next":16' +                
                    '},' + 
                    '{' +
                        '"item-name": "Item Tracking and Digital Cart",' +
                        '"item-image":"TO_ItemTracking.mp4",' + 
                        '"transition-time": 1000,' +                 
                        '"page-number-next":17' +                
                    '},' + 
                    '{' +
                        '"item-name": "Automatic Cashierless Checkout",' +
                        '"item-image":"TO_CashierlessOut.mp4",' +
                        '"transition-time": 1000,' +  
                        '"page-number-next":18' +                
                    '},' + 
                    '{' +
                        '"item-name": "Secure Connections for System Devices",' +
                        '"item-image":"TO_CashierlessSecurity.mp4",' +
                        '"transition-time": 1000,' +  
                        '"page-number-next":19' +                
                    '}' + 
                ']' +
            '},' +
            '{' +
                '"page-number":14,' +
                '"page-number-next":0,' +
                '"page-number-previous":13,' +
                '"page-building":"building1",' +
                '"page-img":"LongQueues.mp4",' +
                '"page-subtitles":{' +
                    '"type":"video",' +
                    '"file":"LongQueues.vtt"' +
                '},' +
                '"page-name":"Convention Center",' +
                '"page-title":"None",' +
                '"page-description":[' +
                '],' +
                '"menu-items": [],' +
                '"page-title-problem":"None",' +
                '"page-description-problem":[],' +
                '"menu-items-problem": []' +
            '},' +
            '{' +
                '"page-number":15,' +
                '"page-number-next":0,' +
                '"page-number-previous":13,' +
                '"page-building":"building1",' +
                '"page-img":"VerizonCashierlessSolution.mp4",' +
                '"page-subtitles":{' +
                    '"type":"video",' +
                    '"file":"VerizonCashierlessSolution.vtt"' +
                '},' +
                '"page-name":"Convention Center",' +
                '"page-title":"None",' +
                '"page-description":[' +
                '],' +
                '"menu-items": [],' +
                '"page-title-problem":"None",' +
                '"page-description-problem":[],' +
                '"menu-items-problem": []' +
            '},' +
            '{' +
                '"page-number":16,' +
                '"page-number-next":0,' +
                '"page-number-previous":13,' +
                '"page-building":"building1",' +
                '"page-img":"Authentication.mp4",' +
                '"page-transition-out":"FROM_Authentication.mp4",' +
                '"page-transition-time":1700,' +
                '"page-subtitles":{' +
                    '"type":"video",' +
                    '"file":"Authentication.vtt"' +
                '},' +
                '"page-name":"Convention Center",' +
                '"page-title":"None",' +
                '"page-description":[' +
                '],' +
                '"menu-items": [],' +
                '"page-title-problem":"None",' +
                '"page-description-problem":[],' +
                '"menu-items-problem": []' +
            '},' +
            '{' +
                '"page-number":17,' +
                '"page-number-next":0,' +
                '"page-number-previous":13,' +
                '"page-building":"building1",' +
                '"page-img":"ItemTracking.mp4",' +
                '"page-transition-out":"FROM_ItemTracking.mp4",' +
                '"page-transition-time":1200,' +
                '"page-subtitles":{' +
                    '"type":"video",' +
                    '"file":"ItemTracking.vtt"' +
                '},' +
                '"page-name":"Convention Center",' +
                '"page-title":"None",' +
                '"page-description":[' +
                '],' +
                '"menu-items": [],' +
                '"page-title-problem":"None",' +
                '"page-description-problem":[],' +
                '"menu-items-problem": []' +
            '},' +
            '{' +
                '"page-number":18,' +
                '"page-number-next":0,' +
                '"page-number-previous":13,' +
                '"page-building":"building1",' +
                '"page-img":"CashierlessOut.mp4",' +
                '"page-transition-out":"FROM_CashierlessOut.mp4",' +
                '"page-transition-time":1000,' +
                '"page-subtitles":{' +
                    '"type":"video",' +
                    '"file":"CashierlessOut.vtt"' +
                '},' +
                '"page-name":"Convention Center",' +
                '"page-title":"None",' +
                '"page-description":[' +
                '],' +
                '"menu-items": [],' +
                '"page-title-problem":"None",' +
                '"page-description-problem":[],' +
                '"menu-items-problem": []' +
            '},' +
            '{' +
                '"page-number":19,' +
                '"page-number-next":0,' +
                '"page-number-previous":13,' +
                '"page-building":"building1",' +
                '"page-img":"CashierlessSecurity.mp4",' +
                '"page-transition-out":"FROM_CashierlessSecurity.mp4",' +
                '"page-transition-time":1000,' +
                '"page-subtitles":{' +
                    '"type":"video",' +
                    '"file":"CashierlessSecurity.vtt"' +
                '},' +
                '"page-name":"Convention Center",' +
                '"page-title":"None",' +
                '"page-description":[' +
                '],' +
                '"menu-items": [],' +
                '"page-title-problem":"None",' +
                '"page-description-problem":[],' +
                '"menu-items-problem": []' +
            '}' +
        ']' +
    '}';

guided_tour = 
    '[' +
        '{' +
            '"step": 0,' +       
            '"time": 20,' +     
            '"page_number": 1,' +
            '"next_page": 2,' +
            '"state": false,' +
            '"building": "building1"' +
        '},' +
        '{' +
            '"step": 1,' +       
            '"time": 11,' +     
            '"page_number": 2,' +
            '"next_page": 3,' +
            '"state": false' +            
        '},' +
        '{' +
            '"step": 2,' +       
            '"time": 5,' +     
            '"page_number": 3,' +
            '"next_page": 4,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 3,' +       
            '"time": 14,' +     
            '"page_number": 4,' +
            '"next_page": 0,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 4,' +       
            '"time": 5,' +     
            '"page_number": 3,' +
            '"next_page": 5,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 5,' +       
            '"time": 6,' +     
            '"page_number": 5,' +
            '"next_page": 0,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 6,' +       
            '"time": 5,' +     
            '"page_number": 3,' +
            '"next_page": 6,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 7,' +       
            '"time": 7,' +     
            '"page_number": 6,' +
            '"next_page": 0,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 8,' +       
            '"time": 5,' +     
            '"page_number": 3,' +
            '"next_page": 7,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 9,' +       
            '"time": 6,' +     
            '"page_number": 7,' +
            '"next_page": 0,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 10,' +       
            '"time": 5,' +     
            '"page_number": 3,' +
            '"next_page": 8,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 11,' +       
            '"time": 7,' +     
            '"page_number": 8,' +
            '"next_page": 0,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 12,' +       
            '"time": 5,' +     
            '"page_number": 3,' +
            '"next_page": 9,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 13,' +       
            '"time": 5,' +     
            '"page_number": 9,' +
            '"next_page": 10,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 14,' +       
            '"time": 25,' +     
            '"page_number": 10,' +
            '"next_page": 0,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 15,' +       
            '"time": 2,' +     
            '"page_number": 9,' +
            '"next_page": 0,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 16,' +       
            '"time": 5,' +     
            '"page_number": 3,' +
            '"next_page": 11,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 17,' +       
            '"time": 5,' +     
            '"page_number": 11,' +
            '"next_page": 12,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 18,' +       
            '"time": 14,' +     
            '"page_number": 12,' +
            '"next_page": 0,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 19,' +       
            '"time": 5,' +     
            '"page_number": 11,' +
            '"next_page": 0,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 20,' +       
            '"time": 5,' +     
            '"page_number": 3,' +
            '"next_page": 0,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 21,' +       
            '"time": 5,' +     
            '"page_number": 3,' +
            '"next_page": 13,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 22,' +       
            '"time": 5,' +     
            '"page_number": 13,' +
            '"next_page": 14,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 23,' +       
            '"time": 8,' +     
            '"page_number": 14,' +
            '"next_page": 0,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 24,' +       
            '"time": 5,' +     
            '"page_number": 13,' +
            '"next_page": 15,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 25,' +       
            '"time": 25,' +     
            '"page_number": 15,' +
            '"next_page": 0,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 26,' +       
            '"time": 5,' +     
            '"page_number": 13,' +
            '"next_page": 16,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 27,' +       
            '"time": 7,' +     
            '"page_number": 16,' +
            '"next_page": 0,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 28,' +       
            '"time": 5,' +     
            '"page_number": 13,' +
            '"next_page": 17,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 29,' +       
            '"time": 11,' +     
            '"page_number": 17,' +
            '"next_page": 0,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 30,' +       
            '"time": 5,' +     
            '"page_number": 13,' +
            '"next_page": 18,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 31,' +       
            '"time": 7,' +     
            '"page_number": 18,' +
            '"next_page": 0,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 32,' +       
            '"time": 5,' +     
            '"page_number": 13,' +
            '"next_page": 19,' +
            '"state": false' +
        '},' +
        '{' +
            '"step": 33,' +       
            '"time": 10,' +     
            '"page_number": 19,' +
            '"next_page": 0,' +
            '"state": false' +
        '}' +
    ']';